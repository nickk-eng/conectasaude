...
function publico(usuario) { 
  return { id: usuario.id, email: usuario.email, nome: usuario.nome, perfil: usuario.perfil, turma: usuario.turma }; }
...
async function autenticar(req, res, next) { 
   usuario = await usuarioLogado(req); if (!usuario) 
    return res.status(401).json({ mensagem: 'Faça login para continuar.' }); req.usuario = usuario; next(); }
function permitir(...perfis) { 
  return (req, res, next) => perfis.includes(req.usuario.perfil) ? next() : res.status(403).json({ mensagem: 'Você não tem acesso a esta área.' }); }
function pagina(arquivo, ...perfis) { 
  return async (req, res) => { 
    const usuario = await usuarioLogado(req); if (!usuario) 
      return res.redirect('/'); if (!perfis.includes(usuario.perfil)) 
        return res.redirect('/home'); res.sendFile(path.join(view, 'html', arquivo)); }; }
...
app.post('/api/auth/sair', async (req, res) => { 
  await banco.query('DELETE FROM sessoes WHERE id = $1', [cookies(req).conectaSaudeSession]);
   res.setHeader('Set-Cookie', 'conectaSaudeSession=; Max-Age=0; Path=/; HttpOnly; SameSite=Strict'); 
   res.status(204).end(); 
  });
...
app.put('/api/aluno/perfil', autenticar, permitir('aluno'), async (req, res) => { 
  ...
  const resultado = await banco.query('UPDATE usuarios SET nome = $1, turma = $2 WHERE id = $3 RETURNING id, email, nome, perfil, turma', [nome, turma, req.usuario.id]); res.json({ usuario: publico(resultado.rows[0])
  }); });
app.get('/api/professor/alunos', autenticar, permitir('professor', 'admin'), async (_req, res) => { 
  const dados = await banco.query("SELECT id, email, nome, perfil, turma FROM usuarios WHERE perfil = 'aluno' ORDER BY nome, email"); res.json({ alunos: dados.rows.map(publico) }); });
...
async function criarAdmin() { 
  if (!process.env.ADMIN_EMAIL || !process.env.ADMIN_SENHA) 
    return; const email = process.env.ADMIN_EMAIL.trim().toLowerCase(); 
  const existe = await banco.query('SELECT id FROM usuarios WHERE email = $1', [email]); 
  if (!existe.rowCount) await banco.query("INSERT INTO usuarios (email, senha_hash, perfil, nome) VALUES ($1, $2, 'admin', 'Administrador')", [email, await hash(process.env.ADMIN_SENHA)]); 
}
